	...
	if(g_MatchStatus==MATCH_IN_ACTION || g_MatchStatus == MATCH_BALL_ON_GOALKEEPER){
		...
		if (ballPossessionByPlayerTeam) {
					...
					if (inRealShootingRange) {
						...
						if (shouldCheck) { 
							...
							if (clearShot) {
								...
								Trampoline_VOID_16_P2(4,PerformShot,shotX, goalTargetY);
								return;
							}
						}
					}
				...
				if (shouldPass) {
					if (!isHumanControlled) {
						...
						for(t=0; t<14; t++) {
                            ...
                            bool isOffside=Trampoline_BOOL_P1(2,IsOffside,t);
						...
						if (bestT != NO_VALUE) {
							...
							if (bestScore > threshold) {
								...
								Trampoline_VOID_P1(2,PerformPass,bestT);
								return; // End tick
							}
						}
					}
			...
			if (g_Ball.PossessionPlayerId == NO_VALUE) {
                ...
                u8 closestId=Trampoline_U8_P2(2,GetClosestPlayerToBall,playerTeamId,NO_VALUE);
		...
		u8 playerClosesestToBallId =Trampoline_U8_P2(2,GetClosestPlayerToBall,playerTeamId,NO_VALUE); 
		...
		if (g_Players[playerClosesestToBallId].Role == PLAYER_ROLE_GOALKEEPER) {
			...
			if (ballFar) {
				u8 nextClosest =Trampoline_U8_P2(2,GetClosestPlayerToBall,playerTeamId,playerClosesestToBallId);
...
void PenaltyShots() {
	...
	Trampoline_VOID(4,ShowHeaderInfo);
    // --- INTRO TEXT ---
    ...
    while(true) {
        ...
        if (kickingTeam == TEAM_1) {
            ...
            while(!shotTaken) {
                ...
                timeoutTimer++;
                bool joyTrigged = Trampoline_U8(2,IsJoystickTriggerPressed);
            ...
            for(u8 w=0; w<120; w++) {
                V9990_WaitSynch();
                u8 joy = Trampoline_U8(2,GetJoystickDirection);
        ...
        if (goal) {
            ...
            V9990_PrintLayerAStringAtPos(12, 10, "GOAL!");
        } else {
            resChar = 'X'; // Uppercase for visibility
            V9990_PrintLayerAStringAtPos(12, 10, saved ? "SAVED!" : "MISS!");
        }
...
void TimeUp(){
    ...
    if (g_Team1Score == g_Team2Score) {
        Trampoline_VOID(5,PenaltyShots);
    } else {
        u8 winner = (g_Team1Score > g_Team2Score) ? TEAM_1 : TEAM_2;
        Trampoline_VOID_P1(5,TeamVictory,winner);
    }
